...
HELP_PAGES: dict[str, dict[str, typing.Any]] = {
    ...
    "info": {
        ...
        "summary": (
            ...
            "  Kernel   Which kernel features the program relies on actually "
            "work here, probed by trying them: the namespaces (created in a "
            "throw-away child), the pseudo-filesystems a login mounts, and "
            "cgroups. Nothing is read from a kernel build config, so a line "
            "says what this kernel does, not what its config claims.\n"
            "  Images   Every installed container with rootfs size, "
            "architecture, image source, busy/idle status, plus source URL and "
            "image type from manifest labels when available. A rootfs holding no "
            "shell to read an architecture out of is answered from the "
            "manifest.\n"
            "  Analysis Lightweight checks per image: architecture mismatch "
            "against the host, a missing manifest, an empty rootfs. An image with "
            "no base distribution at all, a scratch or distroless one, is "
            "reported under Images rather than flagged: it ships no shell by "
            "design and is started with 'run'."
            "\n\n"
            "Read-only, and always run as root: it elevates the way every other "
            "command does, and fails with an error rather than reporting less "
            "when root cannot be obtained."
        ),
        "options": [
            ("-h, --help", "Show this help."),
        ],
