...
[ "$rc" -ne 0 ] || {
	echo "FAIL: a missing COPY source did not fail the build"
	exit 1
}
...
grep -F "Build failed" /tmp/escape-build.log >/tmp/escape-failline.txt ||
	{
		echo "FAIL: the build printed no failure line"
		exit 1
	}
cat -v /tmp/escape-failline.txt
grep -qF 'nope\efile.txt' /tmp/escape-failline.txt ||
	{
		echo "FAIL: the reported name was not escaped"
		exit 1
	}
if grep -qF "$(printf 'nope\033file.txt')" /tmp/escape-failline.txt; then
	echo "FAIL: a raw escape reached the terminal"
	exit 1
fi
[ "$(wc -l </tmp/escape-failline.txt)" -eq 1 ] ||
	{
		echo "FAIL: the failure was not one line"
		exit 1
	}
if grep -q "Traceback" /tmp/escape-build.log; then
	echo "FAIL: the failure left the program as a traceback"
	exit 1
fi
