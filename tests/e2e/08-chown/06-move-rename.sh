...
echo "$out"
[ "$(echo "$out" | wc -l)" -eq 3 ] || {
	echo "FAIL: stat named fewer entries than were moved"
	exit 1
}
bad=$(echo "$out" | awk '$2 != "1500:1600"' | wc -l)
[ "$bad" -eq 0 ] || {
	echo "FAIL: rename(2) writes nothing, and the walk after it missed $bad entries"
	exit 1
}
if sudo chroot-distro login alpine-test -- test -e /tmp/mv-src; then
	echo "FAIL: the source survived the move"
	exit 1
fi
