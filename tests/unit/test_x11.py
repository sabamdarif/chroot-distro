...
def test_resolve_host_x11_env_from_environ(tmp_path):
    ...
    with patch.dict(
        os.environ,
        {
            ...
            "XDG_RUNTIME_DIR": f"/run/user/1000",
        },
        clear=True,
    ), patch("chroot_distro.helpers.x11.resolve_invoking_uid", return_value=1000), patch(
        "chroot_distro.helpers.x11._invoking_home", return_value=str(home)
    ):
...
def test_resolve_host_x11_env_runtime_xauthority_no_extra_bind():
    ...
    with patch.dict(os.environ, {"DISPLAY": ":1", "XAUTHORITY": xauth}, clear=True), patch(
        "chroot_distro.helpers.x11.resolve_invoking_uid", return_value=1000
    ), patch("chroot_distro.helpers.x11._invoking_home", return_value="/home/alice"), patch(
...
def test_resolve_host_x11_env_sudo_fallback(tmp_path):
    ...
    with patch.dict(os.environ, {"SUDO_UID": "1000"}, clear=True), patch(
        "chroot_distro.helpers.x11.resolve_invoking_uid", return_value=1000
    ), patch("chroot_distro.helpers.x11._invoking_home", return_value=str(home)), patch(
        "os.path.isdir", side_effect=lambda p: p == "/run/user/1000" or str(p).endswith("1000")
...
def test_x11_auth_bind_path_rejects_unsafe(tmp_path):
    ...
    with patch("chroot_distro.helpers.x11.resolve_invoking_uid", return_value=1000), patch(
        "chroot_distro.helpers.x11._invoking_home", return_value=str(tmp_path / "home")
    ):
...
def test_get_bindings_x11_auth_binds():
    ...
    with patch("os.path.exists", return_value=True), patch(
        "chroot_distro.commands.login.bindings.IS_TERMUX", False
    ):
        binds = get_bindings(
            rootfs="/fake/rootfs",
            shared_x11=True,
