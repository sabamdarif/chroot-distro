...
#
# COPY/ADD sources are covered by test_copy_parents.py and
# test_tar_extract_safe_resolve.py; this file is about the destination side,
# which runs on the host with nothing confining it: the rootfs a stage is
# assembled in is an ordinary directory, so a symlink the image ships aims any
# write that follows it wherever it likes. A legitimate image ships plenty of
# them (`/var/run -> /run` is in nearly every distro image), so the rule is not
# "refuse symlinks" but "resolve them the way the guest sees them" -- every hop
# re-anchored at the rootfs.
...
def engine(tmp_path, monkeypatch):
    ...
    layer_dir.mkdir()
    monkeypatch.setattr(handlers, "layer_cache_path", lambda digest: str(layer_dir / digest.replace(":", "_")))
...
def _workdir(engine, path):
    handlers.do_workdir(
        engine,
        {"name": "WORKDIR", "value": path, "exec_form": False, "flags": {}, "heredocs": [], "lineno": 1},
    )
...
def test_workdir_absolute_symlink_target_is_re_rooted(engine):
...
def test_materialise_dir_over_a_symlink_does_not_chmod_the_target(tmp_path):
    # `_safe_resolve` covers the *parents* of every entry; the final component is
    # deliberately left alone so the entry itself is replaced rather than written
    # through. A directory entry has to drop a link standing there for that to
    # hold -- os.makedirs(exist_ok=True) is happy with a symlink to a directory
    # and os.chmod() follows one.
    rootfs = tmp_path / "rootfs"
    rootfs.mkdir()
    outside = tmp_path / "outside"
    outside.mkdir(mode=0o700)
    ...
    assert stat.S_IMODE(outside.stat().st_mode) == 0o700
    # The link was replaced by a real directory, as the layer records it and as
    # the tar extractor would apply it.
    made = rootfs / "etc"
...
def test_materialise_tar_dir_member_lands_inside_the_rootfs(tmp_path):
    ...
    outside.mkdir(mode=0o700)
    os.symlink(str(outside), str(rootfs / "etc"))
    spooled = tmp_path / "spool-passwd"
    spooled.write_bytes(b"pwned\n")
