# Containment tests for the two halves of a build step that address the stage
# rootfs after deciding what to touch: the COPY/ADD materialiser (which resolves
# a destination and then writes it) and the layer packer (which lists the tree
# and then reads it).
#
# Both used to name the entry at every step, so the answer they had computed was
# re-resolved by each call that followed it. A process an earlier RUN left
...
def test_materialise_refuses_a_parent_that_is_a_symlink_now(tree, monkeypatch):
    # Stand in for the race: the resolve says <rootfs>/etc, and by the time the
    ...
    with pytest.raises(BuildError) as exc:
        copy_step._materialise_files(str(rootfs), {"etc/passwd": _file_entry(payload)})
...
def test_materialise_does_not_write_through_a_hardlink(tree):
    # O_NOFOLLOW cannot tell a hardlink from an ordinary entry: it *is* the file,
    # under a second name. Writing has to create a new inode.
    rootfs, outside = tree
...
def test_materialise_applies_the_recorded_mode_exactly(tree):
    # open(..., mode) is masked by the umask; the layer records 0666 and the tree
    # has to agree with it.
    rootfs, _outside = tree
    payload = rootfs.parent / "payload"
    payload.write_bytes(b"x")

    copy_step._materialise_files(str(rootfs), {"f": _file_entry(payload, mode=0o666)})

    assert stat.S_IMODE(os.lstat(str(rootfs / "f")).st_mode) == 0o666


def test_materialise_dir_replaces_a_symlink_without_chmodding_its_target(tree):
    rootfs, outside = tree
    os.symlink(str(outside), str(rootfs / "etc"))
...
def test_materialise_symlink_replaces_what_is_in_the_way(tree):
    ...
    copy_step._materialise_files(
        str(rootfs),
        {"link": {"kind": "symlink", "target": "/elsewhere", "mode": 0o777, "uid": 0, "gid": 0, "mtime": 0}},
    )
...
def test_materialise_writes_an_ordinary_tree(tree):
    ...
    copy_step._materialise_files(
        str(rootfs),
        {
            ...
            "opt/app": {"kind": "dir", "mode": 0o700, "uid": 0, "gid": 0, "mtime": 0},
    ...
    assert stat.S_IMODE((rootfs / "opt" / "app" / "run").stat().st_mode) == 0o755


# ── the layer packer ──────────────────────────────────────────────────────────
def _members(path):
    with tarfile.open(path, "r:gz") as tf:
        return {m.name: (m, tf.extractfile(m).read() if m.isreg() else None) for m in tf.getmembers()}
...
def test_layer_omits_an_entry_under_a_symlinked_parent(tree, tmp_path):
    ...
    assert "a/secret" not in _members(str(out))
...
def test_layer_sizes_a_file_from_the_descriptor_it_reads(tree, tmp_path, monkeypatch):
    # tarfile writes exactly tinfo.size bytes from what it is handed, so a size
    # taken off the earlier lstat of the *name* describes a file that may no
    # longer be the one being read.
    rootfs, _outside = tree
    (rootfs / "f").write_bytes(b"short\n")
    stale = tmp_path / "stale"
    stale.write_bytes(b"x" * 4096)
    real_lstat = layer_diff.dirfd.lstat_at

    def stale_lstat(dir_fd, name):
        if name == "f":
            return os.stat(str(stale))
        return real_lstat(dir_fd, name)

    monkeypatch.setattr(layer_diff.dirfd, "lstat_at", stale_lstat)

    out = tmp_path / "layer.tar.gz"
    layer_diff.write_layer_tar(str(rootfs), ["f"], [], str(out))
...
def test_snapshot_handles_a_deep_tree(tree):
    ...
    root_fd = os.open(str(rootfs), os.O_RDONLY | os.O_DIRECTORY)
    try:
        snap = layer_diff.snapshot(str(rootfs))
        assert "/".join(["d"] * 1200) in snap
    finally:
        # pytest's own tmp-dir reaper is shutil.rmtree, which recurses; this tree
        # is deeper than the interpreter's limit on purpose.
        dirfd.rmtree_at(root_fd, "d", force=True)
        os.close(root_fd)
