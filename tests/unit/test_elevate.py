...
def test_get_reexec_argv_python_script():
    ...
    with patch("sys.argv", ["/path/to/main.py", "list"]), \
...
def test_find_escalation_tool_linux():
    ...
    # 1. sudo available
    with patch("shutil.which", side_effect=lambda cmd: "/usr/bin/" + cmd if cmd in ("sudo", "doas", "pkexec", "su") else None):
...
def test_elevate_or_die_exec_sudo():
    mock_exec = MagicMock()
    with patch("chroot_distro.elevate.is_root", return_value=False), \
