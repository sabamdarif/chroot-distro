...
def test_repeated_mount_flags_all_collected():
    mounts = validate_and_parse_run_flags(
        _instr({"mount": ["type=cache,target=/a", "type=tmpfs,target=/b"]})
    )
...
def test_mode_uid_gid_parsed():
    (m,) = validate_and_parse_run_flags(
        _instr({"mount": "type=cache,target=/c,mode=0700,uid=100,gid=200"})
    )
    assert m.mode == 0o700
    assert m.uid == 100
    assert m.gid == 200
...
def test_cache_dir_created_and_lock_held(env, tmp_path):
    ...
    with patch.object(run_mounts, "RUN_CACHE_DIR", cache_root), patch(
        "chroot_distro.locking.RunCacheLock", _FakeLock
    ), patch("chroot_distro.helpers.mount_manager.safe_mount") as sm, patch(
        "chroot_distro.helpers.mount_manager.safe_unmount"
    ):
        with run_mount_session(engine, stage, [m]):
...
def test_cache_sharing_shared_takes_no_lock(env, tmp_path):
    ...
    with patch.object(run_mounts, "RUN_CACHE_DIR", str(tmp_path / "cr")), patch(
        "chroot_distro.locking.RunCacheLock"
    ) as lock_cls, patch("chroot_distro.helpers.mount_manager.safe_mount"), patch(
        "chroot_distro.helpers.mount_manager.safe_unmount"
    ):
        with run_mount_session(engine, stage, [m]):
            pass
...
def test_cache_sharing_private_uses_throwaway_copy(env, tmp_path):
    ...
    with patch.object(run_mounts, "RUN_CACHE_DIR", cache_root), patch(
        "chroot_distro.helpers.mount_manager.safe_mount"
    ) as sm, patch("chroot_distro.helpers.mount_manager.safe_unmount"):
        with run_mount_session(engine, stage, [m]):
            mounted_src = sm.call_args[0][0]
            assert mounted_src.startswith(engine.tmp_root)
        assert not os.path.exists(mounted_src)
...
def test_secret_removed_on_step_failure(env, tmp_path):
    ...
    with patch("chroot_distro.helpers.mount_manager.safe_mount") as sm, patch(
        "chroot_distro.helpers.mount_manager.safe_unmount"
    ), pytest.raises(RuntimeError), run_mount_session(engine, stage, [m]):
...
def test_created_mountpoint_removed_on_teardown(env, tmp_path):
    ...
    with patch("chroot_distro.helpers.mount_manager.safe_mount", side_effect=_placeholder_safe_mount), patch(
        "chroot_distro.helpers.mount_manager.safe_unmount"
    ):
        with run_mount_session(engine, stage, [m]):
